...
# single-threaded:
java $JAVA_OPTS -jar target/benchmarks.jar ArraysSortLongBenchmark -gc $GC -wf $FORK -wi $WITER -w $WTIME -wbs 2 -i $ITER -r $TIME -f $FORK -t 1 $OPTS -jvmArgs "$FORK_OPTS"
