...
public final class DualPivotQuicksort202105 {
    ...
    public static void sortRadixNew(final int[] A) {
        sortRadixNew(A, 0, A.length - 1);
    }

    public static void sortRadixNew(final int[] A, final int low, final int high) {
        final Sorter sorter = SORTER;
        // avoid parallelism on sorter state:
        synchronized (sorter) {
            // preallocation of temporary arrays into custom Sorter class
            ...
            radixSortNew(sorter, A, low, high + 1); // exclusive
        }
    }
     ...
     */
    private DualPivotQuicksort202105() {
    }

    /*
    From Vladimir's source code:
     */
    /**
...
//            count4[(a[i] >>> 24) ^ 0x80]--;
     ...
     * @return true if finally sorted, false otherwise
     */
    static boolean radixSortNewPrev(Sorter sorter, int[] a, int low, int high) {
        if (TRACE) {
            System.out.println("radixSort[" + a.length + "] in [" + low + " - " + high + "]");
        }
        int[] b; int offset = low;
